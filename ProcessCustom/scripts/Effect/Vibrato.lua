local menu_entry = "Vibrato"
local menu_group = "Effect"

Interpolate = require("lib.Interpolation")

local vb = renoise.ViewBuilder()
local rate_ticks      = vb:valuebox { min= 1, max= 20000, value= 12 }
local depth_samples   = vb:valuebox { min= 1, max= 9999,  value= 32 }
...
function main (index, input)
  ...
  for channel, v in pairs(input) do
    out[channel] = Interpolate.BufferHermite(retained_buffer, channel, index + lfo)
  end
